	...
	$('#add-modal .modal-header .title').text('Edit record');
        ...
        inlineSVG($('#title-preview'),'images/designs/'+o.style+'.svg',titleCreator.getTitle(id,true));
	$('#add-modal').modal();
      }
	...
	var k=Object.keys(options);
      ...
      function buildPreview($svg,text=false) {
	if(text===false) {
          ...
          text.style.titleTint=(($('#option-titleFillColor').is(':checked'))?shadeColor2($('#option-primaryColor').val(),0.8):'#ffffff');
          text.artistTint=(($('#option-artistFillColor').is(':checked'))?shadeColor2($('#option-primaryColor').val(),0.8):'#ffffff');
        }
	...
	var options={"x":112.5,"y":19,"text-anchor":"middle","font-size":text.style.font.titleSize+"px","font-family":text.style.font.name,"class":"aside"}
      ...
      function inlineSVG($img,file,text=false) {
